...
function appendSymbol(symbol) {
   ...
   // Проверяем, чему равен последний символ, чтобы не дать пользователю ввести несколько операторов подряд
    ...
    let penultimateSymbol = display.value[display.value.length - 2]; // Предпоследний символ
    ...
    if (operators.includes(lastSymbol) && operators.includes(symbol)){
        ...
        if (symbol === '-') {
            // Если последний символ уже минус — не добавляем второй
            if (lastSymbol === '-') {
                return;
            }
            // Иначе добавляем (это унарный минус после другого оператора)
            display.value += symbol;
        } else {
            // Для других операторов — заменяем последний
            // НО проверяем на случай, когда пользователь сначала ввел унарный минус после оператора,
            // а потом пытается ввести другой оператор
            if (operators.includes(penultimateSymbol)){
                // В таком случае "срезаем" 2 последних символа и добавляем новый
                display.value = display.value.slice(0, -2) + symbol;
            } else {
                // Если нет, то просто заменяем на новый
